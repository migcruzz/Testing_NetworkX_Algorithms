...
from Algorithms.sma_star import sma_star_path, sma_star_path_length
...
class TestSMAStar:
    ...
    def test_astar_directed_weight_function(self):
        ...
        assert sma_star_path(self.XG, "x", "u", weight=w2) == ["x", "y", "s", "u"]
        assert sma_star_path_length(self.XG, "x", "u", weight=w2) == 19
        assert sma_star_path(self.XG, "s", "v", weight=w2) == ["s", "x", "v"]
        assert sma_star_path_length(self.XG, "s", "v", weight=w2) == 10

        def w3(u, v, d):
            return d["weight"] + 10

        assert sma_star_path(self.XG, "x", "u", weight=w3) == ["x", "u"]
        assert sma_star_path_length(self.XG, "x", "u", weight=w3) == 13
        assert sma_star_path(self.XG, "s", "v", weight=w3) == ["s", "x", "v"]
        assert sma_star_path_length(self.XG, "s", "v", weight=w3) == 30
    ...
    def test_astar_directed2(self):
        ...
        assert sma_star_path(XG2, 1, 3) == [1, 4, 5, 6, 3]

    def test_astar_undirected2(self):
        XG3 = nx.Graph()
        edges = [(0, 1, 2), (1, 2, 12), (2, 3, 1), (3, 4, 5), (4, 5, 1), (5, 0, 10)]
        XG3.add_weighted_edges_from(edges)
        assert sma_star_path(XG3, 0, 3) == [0, 1, 2, 3]
        assert sma_star_path_length(XG3, 0, 3) == 15
    ...
    def test_astar_undirected3(self):
        ...
        assert sma_star_path(XG4, 0, 2) == [0, 1, 2]
        assert sma_star_path_length(XG4, 0, 2) == 4
    ...
    def test_astar_directed3(self):
        ...
        graph.add_weighted_edges_from(edges)
        answer = ["n5", "n2", "n1", "n0"]
        assert sma_star_path(graph, "n5", "n0", h) == answer

    def test_sma_star_directed4(self):
        # Definir os valores de heurística
        heuristic_values = {"a": 3, "b": 2, "c": 1, "d": 0, "e": 0}

        def h(u, v):
            return heuristic_values[u]

        # Definir os bordos e o grafo direcionado
        edges = [
            ("a", "b", 1),
            ("a", "c", 1),
            ("b", "d", 2),
        ...
        # Espera-se que o caminho mais curto seja: ["a", "c", "d", "e"]
        expected_path = ["a", "c", "d", "e"]

        # Executar o algoritmo SMA* com a heurística fornecida
        assert sma_star_path(graph, "a", "e", heuristic=h) == expected_path
    ...
    def test_astar_w1(self):
        ...
        assert sma_star_path(G, "s", "v") == ["s", "u", "v"]
        assert sma_star_path_length(G, "s", "v") == 2
    ...
    def test_astar_admissible_heuristic_with_cutoff(self):
        ...
        # assert sma_star_path_length(self.XG, "s", "v") == 9
